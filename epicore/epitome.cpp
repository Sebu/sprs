...
void Chart::caclDimensions() {
    // align
    for(uint i=0; i< reconBlocks_.size(); i++) {
        Patch *block = reconBlocks_[i];
...
cv::Mat Chart::getMap() {
    ...
    std::cout << width << " " << height << " " << minX_ << " " << minY_ << " " << reconBlocks_.size() << std::endl;
...
void Chart::save()
{
    ...
    //*
    cv::Mat tilesImage = cv::Mat::zeros(12 , reconBlocks_.size()*12, CV_8UC3);
