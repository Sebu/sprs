...
reconstruction as a former combination of several orthogonal basis elements.
...
Aharon et al. in 2006\cite{Aharon2006}. The algorithm learns a dictionary
...
\label{sec:mairal}
In 2009 Mairal et. al.\cite{Mairal2009} presented an online approach we will
call \trainDL. The algorithm uses stochastic gradient descent to optimize
...
$\vec{x}_t$ from the training set $\mat{X}$ through a function $p(\vec{x})$.
...
\label{alg:trainDL}
\begin{algorithmic}[1]
\REQUIRE $\mat{X} \in \mathbb{R}^{m \times k},  p \left( \mat{X} \right),
\lambda \in
...
\paragraph{Hierarchy} Jenatton et al. showed in 2010\cite{Jenatton2010} that
