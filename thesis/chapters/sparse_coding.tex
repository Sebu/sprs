...
shown before an $\ell_1$ regularization version of \prettyref{eq:problem} also
keeps
...
regularization version of \prettyref{eq:problem}.
...
Sparse coding is used for the process of remove noise $\vec{w}$ from a signal
$\vec{y}$ gaining the de-noised signal $\vec{x}$. Elad and Aharon showed in
2006\cite{Elad2006}.
\begin{align*}
\vec{y} = \vec{x} + \vec{w}
\end{align*}
