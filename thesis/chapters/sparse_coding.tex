...
While signal $\vec{x}$ and dictionary $\mat{D}$ are known we have to solve an  
under-determined linear system (\prettyref{eq:problem} to obtain the unknown
$\vec{\alpha}$.
\begin{equation}
...
\prettyref{fig:sparse} shows an example of regularization with two coefficients
...
coefficient of $\vec{\alpha}$ \prettyref{eq:mp_update}. The algorithm
re-evaluates
all coefficients in the current set $A$ of active atoms $\vec{\alpha}_A$ by
solving a full least-squares in every iteration (\prettyref{eq:omp_update}).
...
independent variables. Applying this to our problem (\prettyref{eq:problem})
