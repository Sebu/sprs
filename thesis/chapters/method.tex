...
parallel coding\ref{alg:parallel} and the LARS\ref{alg:lars} presented
in\ref{sec:lars}.
...
For 
We chose Batch-OMP  and LARS-Lasso. Batch-OMP as being to solve $\ell_0$
...
different blocks we start to talk about a different game. 
Now each ...


For the learning step we use the \trainDL algorithm.
Robust, convergent, fast, on-line
...
selecting random elements from the training data. Both ways can be tested and 
the quality will be compared.


\subsection{Dict. size}
...
for every batch coding client. \Todo{The problem is that}
...
smooth images animation. Learned dictionaries for very specific classes of
images can be used for classification task.
...
designed. As learned dictionary elements also locality in time, 
But real practical application is an open question,  no quantization
was applied and the. 
...
section\ref{sec:signal_representation}.
