...
better reconstruction quality. This strategy pays more attention to the fact
that color channels are often highly correlated. But they also notices that both
approaches can lead to color bleeding.  
%\Todo{add picture}
%Nevertheless 
They propose a solution that adds a constraint to the sparse coding that pays
...
illustrates this separation step. How the  strategy can effect learning
...
%\subsection*{Software}
The software framework consists of a library (sprscode) for sparse coding image
...
\verb+--output <file+ & output \emph{file} of reconstruction \\ 
