...
For example an uncompressed RGB color image with 8-bit of color depth requires 24-bits per pixel, 
respectively 8-bit for a single channel gray scale image.

Looking at other well known compression algorithms such as jpeg or jpeg2000 a common ratio 
...
The \prettyref{alg:batchOMP} is a modified version of the orthogonal-matching-pursuit (\ref{sec:omp}) 
...
Reduction the complexity to: \Todo{lasso complexity}
...
and a cluster consisting of 100 nodes with each AMD Athlon X2 \Todo{athlon config} running Debian 32-bit. 
