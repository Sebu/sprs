...
image data. As the sparse coding process is operation on samples of signal
...
\begin{figure}[h]
\centering
\subfloat[]{\includegraphics[scale = 0.30]{images/segmentation_new.jpg}}
\hspace{5mm}
...
version of the Orthogonal-Matching-Pursuit(\ref{sec:omp}) named
\emph{Batch-OMP}. The speed gain is achieved by modifying the OMP in a way that
can effectively sparse code multiple signals in parallel. One key element of the
...
Another optimization to a single signal coding step itself
(\prettyref{alg:batchOMP}) is the cholesky factorization of
$\mat{D}_A^T \mat{D}_A$ (line \ref{alg:OMP_DTD} in
\prettyref{alg:mp}). Cholesky factorization is a process of
decomposing a symmetric and positive-definite matrix $\mat{A}$ into
...
an online learning algorithm is able to learn dictionaries from very large
...
skipping this step. %Splitting the image
