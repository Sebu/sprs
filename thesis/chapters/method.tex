...
%, illustrated in fig. \ref{fig:YCbCr},
...
\prettyref{fig:sorted} illustrates an example of this.
...
%As mentioned in \prettyref{sec:headers} we are aware of the fact that besides
...
One of the main questions is if the ODL paper $\ell_1$ regularization is used
does $\ell_0$ also performe good.
Test will be made with OMP and Lasso runs. We also want to know if we can
reconstruct images outsize of the training sets.
...
dictionaries. We compare the reconstruction quality dependency on block size
and average number of coefficients in the learning process.
We want to find good start settings for further experiments with large set of
...
Before we go further and present the results it is necessary to introduce some
additional terms. Such as metrics to measure their performance.
...
in the cluster calculated batches of 100--10,000 images based on the size of
the training sets.
