...
\STATE pre-compute $DtX \gets D^TX$
\FOR {$i = 1$ to $k$}
...
The same pre-computation of the gram matrix $D^TD$ and $D^TX$  used in the
Batch-OMP can also be applied to the LARS-Lasso algorithm. The framework uses a
simple combination of parallel coding\ref{alg:parallel} and the
LARS\ref{alg:lars} presented in\ref{sec:lars}.
...
and good entropy encoding of the data. Both elements led to to the
...
$\ell_0$ also performe good? All test will be made with OMP and Lasso runs.

Further extend them to capture more?
...
Mairal et al. also propose some settings but do they also apply to our problem?
For training sets of $1.25\cdot 10^6$ and about 10 coefficients.
But only dicts with redundancy of 4.
...
Find good settings for large set of different samples.
...
One benefit of the learned dictionary approach is the ability to to learn and
use specific dictionaries for each group of images.
